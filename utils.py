...
def undistort(img):
    # This remains bypassed to avoid the 'cal_pickle.p' FileNotFoundError
    return img 
    # If you eventually get cal_pickle.p, comment the line above and uncomment below
    # with open('cal_pickle.p', mode='rb') as f:
    #     file = pickle.load(f)
    # mtx = file['mtx']
    # dist = file['dist']
...
def sliding_window(img, nwindows=15, margin=50, minpix=1, draw_windows=True):
    ...
    if leftx.size and rightx.size:
        ...
        left_a.append(left_fit[0]); left_b.append(left_fit[1]); left_c.append(left_fit[2])
        right_a.append(right_fit[0]); right_b.append(right_fit[1]); right_c.append(right_fit[2])
        left_fit_ = [np.mean(left_a[-10:]), np.mean(left_b[-10:]), np.mean(left_c[-10:])]
        right_fit_ = [np.mean(right_a[-10:]), np.mean(right_b[-10:]), np.mean(right_c[-10:])]
        ...
        return out_img, (0, 0), (0, 0), np.linspace(0, 1, 10)
...
def draw_lanes(img, left_fit, right_fit, frameWidth, frameHeight, src):
    ...
    # FIXED: Changed np.int_ to int (or just np.int32)
...
def textDisplay(curve, img):
    ...
    if curve > 10: directionText = 'Right'
    elif curve < -10: directionText = 'Left'
...
def drawLines(img, lane_curve):
    myWidth, myHeight = img.shape[1], img.shape[0]
